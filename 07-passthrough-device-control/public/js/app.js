...
const createScene = () => {
  ...
  // 状態(ON/OFF)と往復時間を表示するテキストプレート
  const statusPlane = BABYLON.MeshBuilder.CreatePlane('statusPlane', { width: 1, height: 0.4 }, scene);
  statusPlane.position = new BABYLON.Vector3(0, 1.8, 0);
  statusPlane.billboardMode = BABYLON.Mesh.BILLBOARDMODE_ALL;
  const statusTexture = new BABYLON.DynamicTexture('statusTexture', { width: 512, height: 200 }, scene);
  const statusMat = new BABYLON.StandardMaterial('statusMat', scene);
  statusMat.diffuseTexture = statusTexture;
  statusMat.emissiveColor = new BABYLON.Color3(1, 1, 1);
  ...
  return { scene, cube, cubeMat, statusTexture, clockTexture };
};

const { scene, cube, cubeMat, statusTexture, clockTexture } = createScene();
...
ws.onmessage = (event) => {
  ...
  statusTexture.drawText(
    `${data.state === 'on' ? 'ON' : 'OFF'} (${roundTripMs} ms)`,
    null, null, 'bold 50px monospace', '#FFFFFF', '#333333', true
  );
